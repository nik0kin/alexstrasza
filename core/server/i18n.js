 ...
 * @returns {string}
 */
I18n.t = function t(path, bindings) {
  ...
  if (_.isArray(string)) {
    ...
    string.forEach(function (s) {
      var m = s; // new MessageFormat(s, currentLocale);

      msg.push(m.format(bindings));
    });
  } else {
    msg = string; // new MessageFormat(string, currentLocale);
 ...
 * @returns {string}
 */
I18n.findString = function findString(msgPath) {
